...
### ビューコンポーネントのディレクトリ統合
- **目的**: CalendarViewとListViewを単一のディレクトリにまとめ、関連コンポーネントを整理
- **実装内容**:
  1. **ディレクトリ構造の変更**:
     - `CalendarView/` → `View/CalendarView/` に移動
     - `ListView/` → `View/ListView/` に移動
     - 両ビューコンポーネントをView/ディレクトリ配下に統合
  2. **インポートパスの修正**:
     - SlotSelect.tsx: CalendarViewとListViewのインポートパスを更新
     - useCalendarNavigation.ts: CalendarViewRefのインポートパスを`../View/CalendarView/CalendarView`に変更
     - CalendarView.tsx: typesのインポートパスを`../types` → `../../types`に修正
     - ListView.tsx: typesのインポートパスを`../types` → `../../types`に修正
- **修正ファイル**:
  - ディレクトリ移動:
    - `src/components/pages/SlotSelect/View/CalendarView/` (移動)
    - `src/components/pages/SlotSelect/View/ListView/` (移動)
  - インポート修正:
    - `src/components/pages/SlotSelect/SlotSelect.tsx`
    - `src/components/pages/SlotSelect/hooks/useCalendarNavigation.ts`
    - `src/components/pages/SlotSelect/View/CalendarView/CalendarView.tsx`
    - `src/components/pages/SlotSelect/View/ListView/ListView.tsx`
- **技術的な詳細**:
  - ビューに関連するコンポーネントを単一のディレクトリにまとめることで構造を明確化
  - 相対パスの階層が1つ深くなったため、typesへのインポートパスを調整
- **影響範囲**: ディレクトリ構造の変更のみで、機能に影響なし
- **検証**: `pnpm build`でエラーなくビルドが完了
